...
2. 在生成候选前，可按需读取 `references/model-composition-patterns.md` 做**结构模式识别**。模式只用于提出候选组合及其必需证据，禁止把“样本小/指标多/规模大”等弱线索直接映射为固定算法。
3. 建立 Baseline：应简单、可解释、可快速复现，用于判断复杂方案是否真的带来收益；Baseline 不等于“低质量模型”。
4. 生成少数真正独立的候选模型族。候选之间若只差超参数或同一机制的近似阶次，按一个模型族处理。
5. 逐个检查适用条件：所需数据、变量类型、约束表达能力、样本量、可辨识性、计算复杂度、是否能在剩余环境中实现、可用验证方式。
6. 对候选给出定性证据矩阵：适配度、实现风险、验证成本、解释性、潜在增益。不要用无依据的 0-100“综合分”冒充客观评价。
7. 识别失败模式和退出条件，例如 MILP 规模爆炸、时间序列泄漏、机理参数不可辨识、神经网络样本不足。
8. 给出推荐顺序：Baseline、首选、仅在特定条件下才值得尝试的备选，并说明触发条件。
