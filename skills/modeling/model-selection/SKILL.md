...
1. 先判定每个子问题的主类型：优化、预测、评价、分类/聚类、机理/仿真、图网络、统计/贝叶斯、信号/时频、个体仿真或混合类型；混合题拆成组件而不是硬贴单标签。若赛题属于明显专业领域且领域机制会改变变量、约束或评价口径，先发出 `DOMAIN_CONTEXT_NEEDED`，再做最终模型取舍。
...
3. 建立 Baseline 或最小证明性结果：应简单、可解释，用于判断复杂方案是否真的带来收益；若当前关键未知是可辨识性、参数来源或硬约束，应先解决该未知，再决定是否值得实现 Baseline。
