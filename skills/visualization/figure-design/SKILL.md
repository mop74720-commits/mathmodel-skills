...
2. 识别变量结构：连续/类别/时间/空间/网络/多维；据此选择折线、散点、箱线/区间、热图、地图、网络图等，不根据个人偏好选图。
3. 确定统计口径：均值/中位数、置信区间/分位数、样本量、归一化、基准线、误差定义；图中数值必须由代码计算。
4. 设计主次层级：主结论放最显眼位置，辅助面板只在能解释机制时加入；不要把不同重要性的面板机械等分。
5. 定义坐标轴单位、范围、零点、对数尺度和排序规则；截断坐标必须有合理理由并避免夸大差异。
6. 规划颜色/线型/标记使彩色、灰度、常见色觉缺陷下仍可区分；尽量避免仅靠颜色编码。
7. 写出图注必须包含的最少信息：对象、场景、统计口径、误差条含义、关键参数。
