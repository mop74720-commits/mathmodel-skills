...
按需先读取 `references/figure-guides/pitfalls-and-review.md`，但只把与当前图型和 claim 相关的条目作为攻击面。
...
8. 从整页上下文检查图表是否承担高价值比较：关键结论若只藏在文字而图表只是装饰，应建议重新设计 figure/table storytelling，而不是只修字号。
