# Tool Implementation Matrix — v0.1.13 working tree
...
| Figure | richer CSV/TSV/XLSX profiling; standard scientific figure rendering from a frozen Figure Contract; multi-path raster/SVG/PDF audit; source QA; grayscale/contrast QA; export helper | smoke + actual output review | generic renderer covers standard plots only; complex domain figures still require project-specific code; mechanics do not establish chart semantics/statistics |
...
| Paper Search | OpenAlex + Crossref + Semantic Scholar + arXiv; per-source graceful fallback; DOI + high-threshold fuzzy title dedup; year/citation filters; transparent relevance ranking | network-independent merge/ranking self-test; live sources opportunistic | metadata discovery only; source availability/rate limits vary; paper claims require actual source verification |
...
## 2026-09-10 ScholarSkill gap-audit supplement

- Paper Search expands source diversity from two to four public sources while retaining transparent merge/ranking and graceful fallback.
- Search remains a discovery Tool. Deep reading, method transfer and current-task adoption are handled by `literature-evidence`, not by ranking metadata.
- No third-party ScholarSkill source code or registry entry is copied into this repository.
