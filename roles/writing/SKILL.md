...
## Procedure
1. 先从每个子问题的直接答案和 claim-evidence map 组织正文，再补方法与背景。
2. 关键数字必须能追到正式结果文件；“最优/显著/鲁棒/证明”等强表述必须有同等级证据。
3. 摘要、正文、表图、结论、附录中的模型名、参数、数字、单位做交叉核对。
4. 图表 caption 说明对象、情景、指标和必要单位；图不是装饰，正文必须解释其支持的 claim。
5. 引用先检索候选，再回真实出版页面核验。不得把搜索摘要当原文证据。
6. Word/LaTeX 只改变载体，不得造成公式、编号、引用和科学内容漂移；公式优先保持可编辑。
7. 英文建模论文调用 `writing/english-paper`，完成后再次核对数字、公式、变量和引用。
8. 自审不使用固定“至少 N 图/N 页/N 字”作为通用质量门槛，除非当届官方规则明确要求。
