...
5. 正式运行前按 feature 检查依赖，不要求与当前任务无关的整套环境。Python 与 MATLAB 均可作为正式后端；选择 MATLAB 时按需读取 `roles/coding/references/matlab-implementation.md`，并可用 `roles/coding/scripts/check_matlab_env.m` 记录实际工具箱。
...
## Deep reference
按需读取 `roles/coding/references/numerical-and-reproducibility.md`、`references/solver-robustness.md`；MATLAB 后端额外读取 `roles/coding/references/matlab-implementation.md`。
